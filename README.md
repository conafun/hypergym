...
当前版本：**手机端 v3.1.12**（versionCode 19） / **手环端 v1.0.67**（versionCode 67），底部 5 个页签：**数据 / 肌群 / 日记 / 动作 / 传输**。
...
- **统计卡片**：训练次数、总组数等关键指标。
- **PR 纪录卡片**（在「当天训练卡片」下方）：列出**每个动作的历史最高重量**，紧凑一览。
  - 按**肌群**分组、一组一行，**不写肌群名**——只用该肌群的颜色点表示（与肌群页同一套配色）；组内按**重量降序**，最重的排最前
  - **只显示有配重的动作**：自重（0kg）按口径不算 PR，因此不出现
  - 始终基于**全部历史**，不随页面上方的「周 / 月 / 全部」变化
  - 一组的动作超过一行时自动换行，且**续行缩进对齐**，视觉上明确属于同一组
...
| **应用版本** | 手机 3.1.12 / versionCode 19 ｜ 手环 1.0.67 / versionCode 67 | — |
...
│   │       │   │   ├── MuscleMap.kt      # 动作 → 肌群映射（查手环动作表 + 历史兜底）
│   │       │   │   ├── BandExercises.kt  # 【自动生成】手环动作表镜像，勿手改
...
├── tools/
│   ├── sync-band-exercises.js      # 手环动作表 → 手机端 BandExercises.kt（构建前自动执行）
│   └── add-ps1-bom.ps1             # 给 .ps1 补 UTF-8 BOM（PowerShell 5.1 必需）
...
> 手机端**动作库浏览页**（1324 个动作）与手环动作列表**完全解耦** —— 那是独立的参考教学页面。
> 但**动作 → 肌群的分类**必须与上表逐条一致，见下节。

### 动作分类如何与手环保持一致（自动同步）

手机端记录的每个动作名都来自手环 payload（手机没有手工录入入口），所以「动作属于哪个部位」
必须以手环动作表为准。`data.js` 里每个动作都显式带了 `group`，它就是**唯一权威来源**。

实现方式：**构建期自动生成**，不需要手工同步。

```
miband10pro-trainer/src/common/data.js      ← 手环动作表（唯一权威来源）
        │  tools/sync-band-exercises.js      （build-debug.ps1 每次构建手机端前自动执行）
        ▼
phone-app/.../data/BandExercises.kt          ← 自动生成的 Kotlin 常量（勿手改）
        │
        ▼
phone-app/.../data/MuscleMap.kt              ← 分类：查表 → 历史别名 → 关键词
```

**以后只要在 `data.js` 里增减动作或改 `group`，手机 APK 会跟着变**，无需手动改 Kotlin。
生成器会校验并直接中止构建：动作重名、`group` 不在 `GROUP_ORDER` 里、`data.js` 结构变化等
异常都不会被静默打进包里；动作被移除时还会提醒去 `MuscleMap` 的 `LEGACY_ALIASES`（或关键词）
补上历史映射 —— 旧记录里可能还有那个名字，不补就会掉进「其他」。

手动执行：

```bash
node tools/sync-band-exercises.js            # 生成/更新（打印新增/删除/改分类的差异）
node tools/sync-band-exercises.js --check    # 只校验是否同步，漂移则 exit 1
```

> 为什么生成 Kotlin 而不是 assets JSON：生成 Kotlin 是编译期常量，分类不再需要 `Context`，
> 也不存在「没初始化就静默降级」的顺序依赖。旧实现曾用 1324 条英文动作库做模糊匹配，
> 且 `init()` 只在肌群页被调用过 —— 没进过肌群页就会退化成纯关键词匹配，同一个动作的分类
> 取决于用户点过哪些页面；模糊匹配本身也会误判（26 个动作里有 6 个分错，例如「水平胸推」
> 被匹配成「后水平」而算作**背**）。现在改为精确查表，两个问题一并消失。
...
### 手机端 v3.1.12（versionCode 19） ｜ 手环端 v1.0.67（versionCode 67）

**修复**

- **动作分类与手环不一致**：26 个手环动作里有 **6 个分错部位**，直接导致数据页 PR 卡片、
  肌群占比、数据透视的**分组错乱**。逐条如下：

| 动作 | 手环认定 | 修复前（错） | 出错原因 |
|---|---|---|---|
| 哑铃 | 肩 | 其他 | 被当作「纯器材名」跳过，又无关键词命中 |
| 水平胸推 | 胸 | **背** | 模糊匹配到相似度仅 0.60 的「后水平」 |
| 大剪刀 | 背 | 腿 | 旧别名表把「大剪刀」硬编码成腿，与手环表冲突 |
| 罗马椅 | 核心 | 其他 | 无关键词命中 |
| 腿弯曲 | 腿 | 其他 | 关键词只写了「腿弯举」，漏了「腿弯曲」 |
| 面拉 | 肩 | **背** | 关键词表把「面拉」放在背下 |

- **分类存在「页面顺序依赖」**：`MuscleMap.init()` 只在**肌群页**被调用过，若没进过肌群页，
  分类会静默退化成纯关键词匹配 —— 同一个动作的分类结果取决于用户点过哪些页面。
  且 `groupCache` 是普通 `HashMap`，存在并发写风险。
- **历史动作名不能掉队**：从 git 历史里挖出 `data.js` **曾经出现过的全部 29 个动作名**
  （当前 26 个 + 已移除的 `坐姿胸推`、`胸推`、`肩推`），逐个核对新逻辑 —— 旧记录里仍会有
  这些名字，一个都不许归进「其他」。`胸推` 这类名字的关键词也相应补上了。

**变更**

- 动作分类改为**精确查手环动作表**：`data.js` 的 `DEFAULT_EXERCISES` 是唯一权威来源，
  构建期由 `tools/sync-band-exercises.js` 自动生成为 `BandExercises.kt`。
  **以后改 `data.js` 的动作或 `group`，手机 APK 会跟着变**（`build-debug.ps1` 每次构建前自动同步）。
- 判定顺序改为纯函数，**不依赖任何初始化**：手环动作表 → `LEGACY_ALIASES`（历史动作名）
  → 关键词 → 其他。删除了原来的 1324 条动作库模糊匹配。
- 分组配色与展示顺序也一并取手环 `GROUP_COLORS` / `GROUP_ORDER`，两端彻底一致
  （手环没有的「有氧 / 颈」保留原配色，供历史数据归类）。
- 副作用：肌群页不再为分类解析 941 KB 的 `exercises.json`，少了那次白屏。

**新增**

- `tools/sync-band-exercises.js`：动作表同步工具（支持 `--check` 漂移校验），异常直接中止
  构建，动作被移除时会提醒补 `LEGACY_ALIASES`
- `tools/add-ps1-bom.ps1`：给 `.ps1` 补 UTF-8 BOM（PowerShell 5.1 缺 BOM 会把中文按 ANSI
  解析并报假的语法错误）

### 手机端 v3.1.11（versionCode 18） ｜ 手环端 v1.0.67（versionCode 67）

**新增**

- **数据页新增「PR 纪录」卡片**（在「当天训练卡片」下方）：列出每个动作的历史最高重量
  - 按肌群分组、一组一行，**不写肌群名**，只用该肌群的颜色点表示；组内按重量降序
  - 只显示有配重（`weight > 0`）的动作 —— 自重按口径不算 PR
  - 始终基于全部历史，不随「周 / 月 / 全部」变化
  - 一组动作超过一行时自动换行，续行缩进对齐（用 `Row { 色点 + FlowRow(weight=1f) }` 实现）
- 新增 `StatsEngine.bestWeights(days)`：每个动作的历史最高重量。
  注意它与 `prFlags` 的区别——`prFlags` 是「**哪天**打破了纪录」的**事件表**（日记页徽章用），
  `bestWeights` 是「每个动作**当前是多少**」的**成绩表**（PR 卡片用）
